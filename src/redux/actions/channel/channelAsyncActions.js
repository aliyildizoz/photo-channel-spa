import { getChannelPathById, getChannelCategoriesPath, CHANNEL_PATH, getChannelOwnerPath } from "../../actions/channel/channelEndPoints"
import { getChannelDetailSuccess, getChannelCategoriesSucess, channelCreateSuccess, channelUpdateSuccess, getChannelOwnerSuccess, getPhotoGalerySuccess } from "../../actions/channel/channelActionCreators"
...
export function getChannelCategoriesApi(channelId, history) {
    return async dispatch => {
        await axios.get(getChannelCategoriesPath(channelId)).then(res => {
            console.log(res)
            dispatch(getChannelCategoriesSucess(res.data))
        }).catch(err => {
            console.log(err)
            redirectErrPage(history, err)
        })
