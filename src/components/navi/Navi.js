        ...
        return (
            <div style={{ marginBottom: 55 }}>
                <Navbar collapseOnSelect expand="lg" bg="info" variant="dark" className="fixed-top" >
                    <Container>
                        <Link to="/" style={{ textDecoration: 'none' }} onClick={() => this.props.actions.setSelectedCategories([])}><Navbar.Brand>PhotoChannel</Navbar.Brand></Link>
...
function NaviAuthButtons({ logoutEvent }) {
    ...
    return isLogged ? <Nav >
        <Link to="/" onClick={() => logoutEvent(navigate)} className="nav-link"><i className="fas fa-sign-out-alt"></i> Çıkış</Link>
